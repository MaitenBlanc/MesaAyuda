...
app.post("/api/listarTicket", (req, res) => {
  const { contacto } = req.body;
  console.log("listarTicket: contacto(" + contacto + ")");

  if (!contacto) {
    res
      .status(400)
      .send({ response: "ERROR", message: "contacto no informada" });
    return;
  }

  // scanDbTicket(clienteID).then((resultDb) => {
  //   if (Object.keys(resultDb).length == 0) {
  //     res
  //       .status(400)
  ...
  docClient.scan(params, (err, data) => {
    if (err) {
      return res.status(400).send({ response: "ERROR", message: "DB error" });
    }
    if (data.Items.length === 0) {
      return res.status(200).send({ response: "OK", data: [] });
    }
    res.status(200).send({ response: "OK", data: data.Items });
  });
});
