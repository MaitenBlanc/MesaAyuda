...
document.getElementById("lastlogin").innerHTML =
  "<table><tr><td>Cliente</td><td>" +
  query.id +
  "</td></tr><tr><td>Nombre</td><td>" +
  query.nombre +
  "</td></tr><tr><td>Contacto</td><td>" +
  query.contacto +
  "</td></tr></tr><tr><td>Ultimo ingreso</td><td>" +
  query.fecha_ultimo_ingreso +
  "</td></tr></table>";
...
switch (query.mode) {
  case "LOCAL":
    ...
    console.log(
      "ticket:" + JSON.stringify(ticket) + " options:" + JSON.stringify(options)
    );
  ...
  case "TYPICODE":
    ...
    APIREST_URL =
      "https://my-json-server.typicode.com/lu7did/mesaayuda/posts/" + query.id;
    break;
  case "AWS": // Múltiples casos para la misma acción
    console.log("Utiliza AWS como serverless");
    APIREST_URL =
      "https://n3ttz410ze.execute-api.us-east-1.amazonaws.com/default/listTicketGET?ID=" +
      query.id;
