...
app.post("/api/loginCliente", async (req, res) => {
  ...
  try {
    ...
    if (!clientes || clientes.length === 0) {
      res
        .status(400)
        .send({ response: "ERROR", message: "Cliente no encontrado" });
      return;
    }
    ...
    if (password == paswd) {
      if (activo == true) {
        ...
        res
          .status(400)
          .send(
            JSON.stringify({ response: "ERROR", message: "Cliente no activo" })
          );
      }
    } else {
      res
    ...
    res
      .status(400)
      .send(
        JSON.stringify({ response: "ERROR", message: "DB access error " + err })
      );
  }
});
...
/* 
  CAMBIOS REALIZADOS
  Se modificó la API para que use contacto en lugar de id a la hora de restablecer la contraseña de un cliente
*/
app.post("/api/resetCliente", async (req, res) => {
  ...
  try {
    ...
    const paramsUpdate = {
      TableName: "cliente",
      Key: { id: id },
      UpdateExpression: "SET #p = :p, fecha_cambio_password = :f",
